<?php
/**
 * Kamadenu Goushala Platform - Admin Products Management with Image Upload
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_product') {
    ...
    if (!empty($name) && $categoryId > 0 && $price > 0) {
        ...
        ", [$categoryId, $name, $slug, $sku, $unit, $price, $discountPrice, $stock, $shortDesc, $desc, $finalImage]);

        log_activity((int)$currentUser['id'], 'create_product', 'products', null, "Added store product: {$name} ({$sku})");
        set_flash('success', "Product '{$name}' created successfully.");
        header('Location: ' . BASE_URL . '/admin/products.php');
        exit;
    } else {
        set_flash('danger', 'Please fill in all mandatory product fields.');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_product') {
    ...
    if ($product) {
        ...
        log_activity((int)$currentUser['id'], 'delete_product', 'products', $prodId, "Deleted product ID {$prodId}");
...
<div class="card p-4 rounded-4 border-0 shadow-sm bg-white mb-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <div>
            <h1 class="h4 font-serif text-forest-dark mb-0">Organic Store Products & Inventory</h1>
        ...
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                <?php if (empty($products)): ?>
                    <tr>
                        <td colspan="6" class="text-center py-4 text-muted">No products cataloged yet.</td>
                    </tr>
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td class="text-end">
                            <form method="POST" action="" onsubmit="return confirm('Are you sure you want to delete this product?');" class="d-inline">
                                ...
                                <input type="hidden" name="product_id" value="<?= $p['id']; ?>">
                                <button type="submit" class="btn btn-outline-danger btn-sm rounded-pill px-2 py-0" title="Delete Product">
