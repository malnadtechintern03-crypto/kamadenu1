...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $vetName = sanitize_input($_POST['vet_name'] ?? 'Dr. H. V. Narayana (MVSc)');
    $diagnosis = sanitize_input($_POST['diagnosis'] ?? '');
    $treatment = sanitize_input($_POST['treatment'] ?? '');
    $medicine = sanitize_input($_POST['medicine_prescribed'] ?? '');
    $visitDate = sanitize_input($_POST['visit_date'] ?? date('Y-m-d'));
    $nextVisit = sanitize_input($_POST['next_visit_date'] ?? '') ?: null;
    $notes = sanitize_input($_POST['notes'] ?? '');

    if ($cowId > 0 && !empty($diagnosis) && !empty($treatment)) {
        Database::insert("
            INSERT INTO cow_medical_records (cow_id, vet_name, diagnosis, treatment, medicine_prescribed, visit_date, next_visit_date, notes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ", [$cowId, $vetName, $diagnosis, $treatment, $medicine, $visitDate, $nextVisit, $notes]);

        set_flash('success', 'Veterinary medical treatment record logged successfully.');
        header('Location: ' . BASE_URL . '/admin/medical.php');
        exit;
    } else {
        set_flash('error', 'Please select a cow and enter diagnosis/treatment.');
    }
}
        ...
        <table class="table table-hover align-middle mb-0 small">
            ...
            <tbody>
                <?php if (empty($records)): ?>
                    <tr><td colspan="7" class="text-center py-4 text-muted">No medical records logged yet.</td></tr>
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td><?= e($r['treatment']); ?></td>
                        <td><?= e($r['medicine_prescribed'] ?? 'N/A'); ?></td>
                        <td class="text-nowrap"><?= $r['next_visit_date'] ? format_date($r['next_visit_date']) : '<span class="text-muted">—</span>'; ?></td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
                ...
                <div class="modal-body p-4">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="vet_name" class="form-control" value="Dr. H. V. Narayana (MVSc)" required>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" name="medicine_prescribed" class="form-control" placeholder="e.g. Meloxicam 100ml, Liv-52 Liquid 50ml daily">
                        </div>
                        <div class="col-md-6">
